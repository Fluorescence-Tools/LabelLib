...
def main(args):
    ...
    for frIdx in range(traj.n_frames):
        ...
        gridAcceptor = genAV(
            traj, frIdx, acceptorAttIdx, 22.0, 2.0, 3.5, 0.9, 0.4, 3.5 + 3.0
        )
        ...
        distances = ll.sampleDistanceDistInv_arr(gridDonor, gridAcceptor, 1000000)
...
def genAV(
    ...
    av = ll.dyeDensityAV1_arr(atoms, source, length, width, radius, resolution)
    if cvFrac is None:
        return av
    # ACV
    ...
    acv = ll.addWeights_arr(av, surfaceAtoms)
