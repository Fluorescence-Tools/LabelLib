...
av1 = ll.dyeDensityAV1_arr(
    atoms, source, linker_length, linker_width, dye_radius_1, simulation_grid_spaceing
)
minLengthGrid = ll.minLinkerLength_arr(
    atoms, source, linker_length, linker_width, dye_radius_1, simulation_grid_spaceing
)
...
Emean = ll.meanEfficiency_arr(av1, av1, 52.0, 100000)
print("Emean = {:.3f}".format(Emean))

# Contact volume (re)weigting
labels = np.full(
    [1, atoms.shape[1]], 10.123
)  # density close to surfaceAtoms will be 10.123 units higher
surfaceAtoms = np.vstack([atoms, labels])
surfaceAtoms[3] += 2.34  # contact radius is larger than vdW radius
acv = ll.addWeights_arr(av1, surfaceAtoms)
savePqr("ACV.pqr", acv)

# Distance distribution
distances = ll.sampleDistanceDistInv_arr(acv, acv, 1000000)
